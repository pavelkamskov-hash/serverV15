...
LOGIN_PATH = os.path.join(BASE_DIR, 'config.js')
...
def load_login_config():
    """Load login username and bcrypt hash from config.js."""
    try:
        with open(LOGIN_PATH, 'r', encoding='utf-8') as f:
            text = f.read()
        user = re.search(r"username:\s*['\"]([^'\"]+)['\"]", text)
        pw = re.search(r"passwordHash:\s*['\"]([^'\"]+)['\"]", text)
        return {
            'username': user.group(1) if user else '',
            'passwordHash': pw.group(1) if pw else '',
        }
    except Exception:
        return {'username': 'zavod', 'passwordHash': ''}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not sess or sess.get('user') != login_cfg.get('username'):
    ...
    def handle_login(self, data):
        ...
        if (
            username_in == login_cfg.get('username')
            and bcrypt.checkpw(password_in.encode('utf-8'), login_cfg.get('passwordHash', '').encode('utf-8'))
        ):
            sid = generate_sid()
            with SESSION_LOCK:
